...
def Train(MAXN, S = 0):
    ...
    torch.save(model.state_dict(), 'checkpoint/nn_killpriorts-5.pkl')
    pickle.dump([C, Range], open('checkpoint/nn_killpriorts-5.norm', "w"))
    pickle.dump(loss_map, open('checkpoint/nn_killpriorts-5.loss', "w"))
    plt.plot(loss_map)
    plt.savefig("loss_nn_killpriorts5.png")
    plt.close()

def test_match(matchid):
    model = Net(DIM * W, 1)
    model.load_state_dict(torch.load('checkpoint/nn_killpriorts-5.pkl'))
    [C, Range] = pickle.load(open('checkpoint/nn_killpriorts-5.norm', "r"))
...
def test(S, E):
    ...
    plt.plot(percent)
    pickle.dump(percent, open('checkpoint/nn_killpriorts-5.percent', 'w'))
